...
export function WhyThisWinner({ q }: Props) {
  if (!q.recommendationReasoning || !q.recommendedNegotiationId) return null;
  const comparison = q.recommendationComparison ?? [];
  const winner = comparison.find((c) => c.negotiationId === q.recommendedNegotiationId);
  ...
  return (
    <section className="rounded-xl border border-border bg-card p-6">
      ...
      <p className="mt-4 max-w-3xl text-[15px] leading-[1.7] text-foreground">
...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <div className="font-mono text-[12.5px] text-foreground tabular">
        {value}
      </div>
    </div>
  );
